...
class Ukf_odom_imu(Node):
    def __init__(self):
        super().__init__('ukf_odom_imu')
        ...
        self.sub_ukf = self.create_subscription(Odometry, '/odom/ukf_odom', self.ukf_callback, 10)
        self.sub_ukf_imu = self.create_subscription(Odometry, '/odom/ukf_odom_imu', self.ukf_imu_callback, 10)
        ...
        self.angulo_objetivo = math.pi / 2.1
        self.lado_actual = 0
        ...
        self.get_logger().info("Nodo Ukf_odom_imu iniciado")
    ...
    def ukf_callback(self, msg):
        ...
        self.tray_ukf.append((x, y))

    def ukf_imu_callback(self, msg):
        x = msg.pose.pose.position.x
        y = msg.pose.pose.position.y
        self.tray_ukf_imu.append((x, y))
    ...
    def generar_grafica(self):
        ...
        if self.tray_ukf:
            x_ukf, y_ukf = zip(*self.tray_ukf)
            plt.plot(x_ukf, y_ukf, 'g-', linewidth=2, label='UKF (odom)')

        if self.tray_true:
            x_true, y_true = zip(*self.tray_true)
            plt.plot(x_true, y_true, 'r--', linewidth=2, label='Verdadera (TF)')

        if self.tray_ukf_imu:
            x_ukf_imu, y_ukf_imu = zip(*self.tray_ukf_imu)
            plt.plot(x_ukf_imu, y_ukf_imu, 'c--', linewidth=2, label='UKF (odom+imu)')
