...
class Ukf_odom(Node):
    def __init__(self):
        super().__init__('ukf_odom')

        # Publicador de cmd_vel
        self.publisher = self.create_publisher(Twist, '/cmd_vel', 10)

        # Suscripciones para odometría y EKF
        self.sub_odom = self.create_subscription(Odometry, '/odom', self.odom_callback, 10)
        self.sub_ekf = self.create_subscription(Odometry, '/odom/ukf_odom', self.ukf_callback, 10)
        ...
        self.get_logger().info("Nodo Ukf_odom iniciado")
    ...
    def generar_grafica(self):
        ...
        if self.tray_ekf:
            ...
            plt.plot(x_ekf, y_ekf, 'g-', linewidth=2, label='UKF (odom)')
